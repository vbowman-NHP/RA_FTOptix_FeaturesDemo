...
public class ExclusiveLevelAlarmWithEmailNotificationLogic : BaseNetLogic, IUAEventObserver {
    public override void Start() {
        ...
        if (emailVariable == null) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Could not find Email variable of {emailUserNode.BrowseName} user");
            return;
        }

        email = emailVariable.Value;
        if (string.IsNullOrEmpty(email)) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Email variable property missing in user {Log.Node(emailUserNode)} set in {Log.Node(alarmObject)}");
            return;
        }
    }
    ...
    private void SendAlarmEmail(string alarmBrowseName, string alarmMessage) {
        ...
        if (emailSender == null) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", "Could not send email: Invalid or missing EmailSender NetLogic");
            return;
        }
        ...
        Log.Info("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Sending email to {email}");
    ...
    private IUANode GetAlarmProperty(string propertyName) {
        ...
        if (property == null) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Alarm property {propertyName} could not be found");
            return null;
        }

        var propertyValue = (NodeId)property.Value;
        if (propertyValue == NodeId.Empty || propertyValue == null) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Invalid or missing value for alarm property {propertyName}");
            return null;
        }

        var pointedNode = InformationModel.Get(propertyValue);
        if (pointedNode == null) {
            Log.Error("ExclusiveLevelAlarmWithEmailNotificationLogic", $"Could not resolve alarm property {propertyName}");
            return null;
        }
